...
import cz.dp.share.entity.Zastup;
...
public interface ZastupRepository extends JpaRepository<Zastup, String> {
	...
	@Query("select case when count(zastup) > 0 then true else false end from Zastup zastup where zastup.uzivatelZastupce.idUzivatel = :idUzivatel and zastup.uzivatel.idUzivatel = :idZastup and zastup.platnostOd <= CURRENT_TIMESTAMP() and zastup.platnostDo >= CURRENT_TIMESTAMP() and zastup.aktivita = true")
