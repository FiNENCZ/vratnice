...
import cz.dp.share.repository.UzivatelModulRepository;
		...
		http.sessionManagement(cust -> cust.sessionCreationPolicy(SessionCreationPolicy.STATELESS))
				...
				.csrf(cust -> cust.disable()).logout(cust -> {
					...
					cust.requestMatchers("/login", "/*/login", "/login-test", "/*/login-test", "/login-sso-complete",
							"/*/login-sso-complete", "/username-by-rfid", "/username-by-rfid-uid",
							"/*/username-by-rfid", "/*/username-by-rfid-uid", "/login-token", "/*/login-token",
							"/verify", "/*/verify",
							"/api/public/**", "/api/test/**",
