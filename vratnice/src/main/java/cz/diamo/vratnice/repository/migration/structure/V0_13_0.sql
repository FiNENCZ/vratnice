-- Zvětšení hodnoty pro uživatelské nastavení
ALTER TABLE vratnice.uzivatelske_nastaveni 
ALTER COLUMN hodnota TYPE text;

-- Doplnění sloupců pro uživatele
ALTER TABLE vratnice.uzivatel ADD ulice varchar(400);
ALTER TABLE vratnice.uzivatel ADD psc varchar(400);
ALTER TABLE vratnice.uzivatel ADD obec varchar(400);
ALTER TABLE vratnice.uzivatel ADD cislo_popisne varchar(400);
ALTER TABLE vratnice.uzivatel ADD dilci_personalni_oblast varchar(100);
...
insert into vratnice.zdrojovy_text (hash, text) values ('COLUMN_NAME_ULICE','Ulice');
insert into vratnice.zdrojovy_text (hash, text) values ('COLUMN_NAME_PSC','PSČ');
insert into vratnice.zdrojovy_text (hash, text) values ('COLUMN_NAME_OBEC','Obec');
insert into vratnice.zdrojovy_text (hash, text) values ('COLUMN_NAME_CISLO_POPISNE','Číslo popisné');
insert into vratnice.zdrojovy_text (hash, text) values ('COLUMN_NAME_DILCI_PERSONALNI_OBLAST','Dílčí personální oblast');

-- Verze DB
update vratnice.databaze set verze_db = 0, sub_verze_db = 13, cas_zmn = now(), zmenu_provedl = 'pgadmin';