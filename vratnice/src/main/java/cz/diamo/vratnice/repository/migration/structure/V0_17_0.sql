-- Na uživatele doplněn příznak pružné pracovní doby
alter table vratnice.uzivatel add column pruzna_prac_doba boolean not null default false;

-- Nastavení příznaku dle kmenových dat
update vratnice.uzivatel set pruzna_prac_doba = (select k.pruzna_prac_doba from vratnice.kmenova_data k where k.id_kmenova_data = uzivatel.id_kmenova_data) where id_kmenova_data is not null;
...
insert into vratnice.zdrojovy_text (hash, text) values ('COLUMN_PRUZNA_PRAC_DOBA','Pružná pracovní doba');

-- Verze DB
update vratnice.databaze set verze_db = 0, sub_verze_db = 17, cas_zmn = now(), zmenu_provedl = 'pgadmin';