...
public interface UzivatelRepository extends JpaRepository<Uzivatel, String> {
        ...
        @Query(sqlSelect
                        + "where s.aktivita = true and (upper(substring(s.cip1, 3, 8)) like upper(:rfidUid) or upper(substring(s.cip2, 3, 8)) = upper(:rfidUid)) order by s.idUzivatel DESC limit 1")
